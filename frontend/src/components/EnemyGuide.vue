...
const props=defineProps<{config:Config;unlocked?:boolean}>();const emit=defineEmits<{close:[]}>();
...
const available=computed(()=>Object.entries(props.config.enemies).filter(([id])=>id!=='leviathan'||props.unlocked));
watch(()=>props.unlocked,()=>{if(!props.unlocked&&selected.value==='leviathan')selected.value='grunt';});
const entries=computed(()=>available.value.filter(([,p])=>filter.value==='all'||(filter.value==='boss'?p.boss:filter.value==='air'?p.air&&!p.boss:!p.air&&!p.boss)));
