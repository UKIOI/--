...
export const enemyLore:Record<string,{role:string;behavior:string;counter:string}>={
 burrow_nest:{role:'地底孵化精英',behavior:'6 分钟后出现，基础生命 1500、体型 1.8×2 格、护甲 10%，最多同时 2 只。前 10 分钟刷出权重为 25%，10—15 分钟渐增至正常，战役也适用。锁定建筑底部，预警 6 秒后破土，直接摧毁本体出土范围内建筑并造成周边范围伤害，释放 4 只虫；之后每 12 秒生成 2 只。',counter:'潜地时无法攻击；按预警吊装关键设施，出土后用单体火力优先消灭本体，阻止持续增援。'},
 ...
 fortress:{role:'重型空投平台',behavior:'7 分钟后出现，基础生命 1800、护甲 20%，最多同时 2 艘。进入战区后空投 4 只地面虫，之后每 18 秒增援；被击毁后预警坠落，坠爆对建筑和核心的基础伤害 90、对地面虫群为 180，范围 4 格。',counter:'利用坠爆清理下方虫群；撤离残骸落点。指挥虫在场时堡垒会吸引自动防空，手动瞄准可攻击其他空中敌人。'},
