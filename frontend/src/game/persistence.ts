...
export class SaveQueue {
 ...
 private async drain(){let success=true;while(this.pending){const s=this.pending;this.pending=null;s.revision=this.revision;try{await localPut('save',s);}catch{this.status('浏览器备份失败，请勿关闭页面');success=false;continue;}if(this.blocked){this.status('存档冲突：已保留浏览器副本');success=false;continue;}try{const {data}=await api<{revision:number}>('/save','PUT',{expectedRevision:this.revision,snapshot:s});this.revision=data.revision;s.revision=data.revision;await localPut('save',s);this.updated(data.revision);this.status(import.meta.env.VITE_PORTABLE==='true'?'已保存 · 当前浏览器':'已保存 · 本机与浏览器');}catch(e){success=false;if(e instanceof ApiError&&e.status===409){this.blocked=true;this.conflict();}this.status(e instanceof ApiError?e.message:'服务离线 · 已保留浏览器副本');}}return success;}
}
