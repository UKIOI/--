...
test('field guide shows all distinct portraits, filters and sound preview',async({page},testInfo)=>{
 ...
 expect(hashes.size).toBe(Object.keys(config.enemies).length);await page.getByRole('button',{name:'Boss',exact:true}).click();await expect(page.locator('.guide-list button')).toHaveCount(2);
